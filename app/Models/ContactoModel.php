...
class ContactoModel extends DBAbstractModel 
{
    ...
    */
    ...
    {
        try {
            $this->query = "SELECT * FROM contactos WHERE nombre LIKE :nombre OR email LIKE :email OR telefono LIKE :telefono";
            $this->parametros['nombre'] = "%$filter%";
            $this->parametros['email'] = "%$filter%";
            $this->parametros['telefono'] = "%$filter%";
            $this->get_results_from_query();
            return $this->rows;
